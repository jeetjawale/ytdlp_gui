    ...
    def run(self):
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                if not thumb_url:
                    # For playlists, try first entry's thumbnail
                    ...
                    if thumbnails:
                        ...
                        thumb_url = (
                            first.get("thumbnail", "")
                            or (first.get("thumbnails", [{}])[-1].get("url", "")
                                if first.get("thumbnails") else "")
                        )
...
class DownloadWorker(QThread):
    ...
    def _progress_hook(self, d: dict):
        ...
        if status == "downloading":
            ...
            playlist_index = info.get("playlist_autonumber") or info.get("playlist_index")
